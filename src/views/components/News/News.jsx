  ...
  return (
    <div className="content-slider-container my-4">
      ...
      >
        <h3 className="mb-0 fs-2 fw-bolder">{title}</h3>
        <div className="slider-controls">
          <button className="btn btn-outline-light btn-sm me-2 custom-prev">
            <FaChevronLeft />
          </button>
          <button className="btn btn-outline-light btn-sm custom-next">
            <FaChevronRight />
          </button>
        </div>
      </div>
      ...
      <Swiper
        ...
        autoplay={autoSlide ? { delay: autoSlideInterval } : false}
        loop={true}
        navigation={{
          nextEl: ".custom-next",
          prevEl: ".custom-prev",
      ...
      >
        {items.map((item, index) => (
          <SwiperSlide key={index}>
            <div
              className="card border-0 shadow-sm m-2"
              style={{ height: "450px" }}
            >
              {item.image && (
                <img
              ...
              <div className="card-body d-flex flex-column">
                ...
                {item.excerpt && (
                  <p className="card-text text-muted flex-grow-1">
                    {item.excerpt.substring(0, 135)}
                    {item.excerpt.length > 135 ? "..." : ""}
                  </p>
                )}
                <div className="mt-auto">
                  <button
